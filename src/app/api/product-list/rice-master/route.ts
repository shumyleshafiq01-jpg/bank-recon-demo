...
import { RICE_DEFAULT_BYPRODUCTS, RICE_DEFAULT_CHARGES } from "@/lib/rice-costing";

// Rice "Master Prices": the shared rate table that drives every rice product's
// costing. Two kinds of row:
//   byproduct — PKR/kg resale value of a milling by-product (BROKEN/B2, CSR…)
...
function parseRow(r: string[]) {
  return { id: r[0] ?? "", kind: (r[1] ?? "charge") as "byproduct" | "charge", name: r[2] ?? "", rate: parseFloat(r[3]) || 0, sortOrder: parseInt(r[4]) || 0 };
}

// First time the Rice division is opened there's nothing in the sheet — seed it
// with the sample rates so the accountant has a recognisable starting point.
async function ensureSeeded() {
  const rows = await readSheet(SHEET);
  if (rows.length > 1) return;
  const seed: string[][] = [];
  RICE_DEFAULT_BYPRODUCTS.forEach((b, i) => seed.push([genId(), "byproduct", b.name, String(b.rate), String(i)]));
