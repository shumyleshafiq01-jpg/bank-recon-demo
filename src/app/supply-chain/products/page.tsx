  ...
  return (
    <div className="min-h-screen" style={{ background: "#0c1220" }}>
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-bold text-white">Product Master</h1>
              <span className="text-xs text-gray-600">{products.length} products</span>
            </div>
          </div>
          <button onClick={openCatalog} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-colors cursor-pointer">
            <Plus className="w-4 h-4" /> Add from Product List
          </button>
        </div>

        {/* Info banner */}
        <div className="mb-4 flex items-start gap-2 px-4 py-2.5 rounded-lg bg-blue-500/[0.06] border border-blue-500/20 text-xs text-blue-300/90">
          <Info className="w-4 h-4 shrink-0 mt-0.5 text-blue-400" />
          <p>Products are created in the <span className="font-medium text-blue-200">Product List</span> (Foods &amp; Spices, Rice, etc.). Add them here, then fill the carton dimensions &amp; container capacities manually.
            {needsSpecs > 0 && <span className="text-amber-300"> {needsSpecs} product{needsSpecs > 1 ? "s" : ""} still need carton specs.</span>}
          </p>
        </div>
        ...
        {editing && form && (
          <div className="mb-4 p-5 rounded-xl bg-white/[0.04] border border-emerald-500/20">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-white font-semibold text-sm">Carton Specs</h3>
              <button onClick={() => { setEditing(null); setForm(null); }} className="p-1 rounded hover:bg-white/5 text-gray-500 cursor-pointer"><X className="w-4 h-4" /></button>
            </div>
            <p className="text-xs text-gray-500 mb-4">
              <span className="text-white font-medium">{editing.product_name}</span>
              {editing.source_division && <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400">{editing.source_division === "rice" ? "Rice" : editing.source_division === "foods_spices" ? "Foods & Spices" : editing.source_division}</span>}
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
              <div className="col-span-2">
                <label className="text-xs text-gray-500 mb-1 block">Packing Description</label>
                <input value={form.packing_desc} onChange={e => setForm(f => f && ({ ...f, packing_desc: e.target.value }))} placeholder="e.g. 150G X 48 POUCH IN CARTON" className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white placeholder-gray-700 focus:outline-none focus:border-emerald-500/50" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Pcs/Carton</label>
                <input type="number" value={form.pcs_per_carton || ""} onChange={e => setForm(f => f && ({ ...f, pcs_per_carton: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Net Weight/Carton (kg)</label>
                <input type="number" step="0.01" value={form.net_weight_kg || ""} onChange={e => setForm(f => f && ({ ...f, net_weight_kg: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
            </div>
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-4">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">L (in)</label>
                <input type="number" step="0.01" value={form.length_in || ""} onChange={e => setForm(f => f && ({ ...f, length_in: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">W (in)</label>
                <input type="number" step="0.01" value={form.width_in || ""} onChange={e => setForm(f => f && ({ ...f, width_in: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">H (in)</label>
                <input type="number" step="0.01" value={form.height_in || ""} onChange={e => setForm(f => f && ({ ...f, height_in: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Max 20FT</label>
                <input type="number" value={form.max_20ft || ""} onChange={e => setForm(f => f && ({ ...f, max_20ft: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Max 40FT</label>
                <input type="number" value={form.max_40ft || ""} onChange={e => setForm(f => f && ({ ...f, max_40ft: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Max 40HC</label>
                <input type="number" value={form.max_40hc || ""} onChange={e => setForm(f => f && ({ ...f, max_40hc: Number(e.target.value) }))} className="w-full bg-white/[0.06] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => { setEditing(null); setForm(null); }} className="px-4 py-1.5 rounded-lg text-sm text-gray-400 hover:text-white cursor-pointer">Cancel</button>
                ...
                {filtered.map((p, i) => {
                  ...
                  return (
                    <tr key={p.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-2.5"><span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400">{p.brand || "—"}</span></td>
                      <td className="px-4 py-2.5 text-white text-xs font-medium">
                        {p.product_name}
                        {missing && <span className="ml-2 inline-flex items-center gap-0.5 text-[10px] text-amber-400"><AlertTriangle className="w-3 h-3" /> needs specs</span>}
                      </td>
                      <td className="px-4 py-2.5 text-gray-500 text-xs">{p.packing_desc || "—"}</td>
                      <td className="px-4 py-2.5 text-center text-gray-400 text-xs">{p.length_in || "—"}</td>
                      <td className="px-4 py-2.5 text-center text-gray-400 text-xs">{p.width_in || "—"}</td>
                      <td className="px-4 py-2.5 text-center text-gray-400 text-xs">{p.height_in || "—"}</td>
                      <td className="px-4 py-2.5 text-center text-emerald-400 text-xs font-medium">{p.max_20ft || "—"}</td>
                      <td className="px-4 py-2.5 text-center text-blue-400 text-xs font-medium">{p.max_40ft || "—"}</td>
                      <td className="px-4 py-2.5 text-center text-violet-400 text-xs font-medium">{p.max_40hc || "—"}</td>
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1">
                          <button onClick={() => startEdit(p)} className="p-1 rounded hover:bg-white/5 text-gray-600 hover:text-white cursor-pointer" title="Fill/edit carton specs"><Edit3 className="w-3.5 h-3.5" /></button>
                          <button onClick={() => deleteProduct(p.id)} className="p-1 rounded hover:bg-red-500/10 text-gray-600 hover:text-red-400 cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
                {filtered.length === 0 && <tr><td colSpan={11} className="px-4 py-8 text-center text-gray-600 text-sm">No products yet. Click &quot;Add from Product List&quot; to begin.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-6" onClick={() => setShowCatalog(false)}>
          <div className="bg-[#141c2e] border border-white/[0.1] rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
              <div>
                <h3 className="text-white font-semibold">Add from Product List</h3>
                <p className="text-xs text-gray-500 mt-0.5">Only products created in the Product List appear here</p>
              </div>
              <button onClick={() => setShowCatalog(false)} className="p-1 rounded hover:bg-white/5 text-gray-500 cursor-pointer"><X className="w-4 h-4" /></button>
            </div>
            <div className="px-5 py-3 flex gap-2 border-b border-white/[0.06]">
              ...
              <div className="relative">
                <select value={catalogDivision} onChange={e => setCatalogDivision(e.target.value)}
                  className="bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-white appearance-none pr-8 focus:outline-none cursor-pointer">
                  {divisions.map(d => <option key={d} value={d} className="bg-[#141c2e]">{d}</option>)}
                </select>
                <ChevronDown className="w-3.5 h-3.5 absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" />
              </div>
            </div>
              ...
              {filteredCatalog.map(item => {
                ...
                return (
                  <button key={item.sourceProductId} onClick={() => !already && addFromCatalog(item)} disabled={already || isAdding}
                    className={`w-full flex items-center gap-3 px-5 py-3 border-b border-white/[0.04] text-left transition-colors ${already ? "opacity-40 cursor-default" : "hover:bg-white/[0.04] cursor-pointer"}`}>
                    ...
                    {isAdding && <Loader2 className="w-4 h-4 animate-spin text-emerald-400 shrink-0" />}
                    {already && !isAdding && <Check className="w-4 h-4 text-emerald-400 shrink-0" />}
                  </button>
                );
              })}
